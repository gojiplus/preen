                    ...
                    if module_name:
                        ...
                        if imp.startswith(".."):
                            ...
                            if len(parts) > 1:
                                base = ".".join(parts[:-1])
                                target = imp[2:]  # Remove '..'
                                full_import = f"{base}.{target}" if target else base
                            else:
            ...
            if len(cycle) > 1:  # Only report actual cycles
                ...
                issues.append(
                    Issue(
                        ...
                        explanation=(
                            "Circular imports can cause runtime failures when "
                            "modules are loaded. They make code harder to "
                            "understand and test."
                        ),
                        override_question=(
                            "This is a serious issue that can break your "
                            "package. Are you sure you want to release anyway?"
                        ),
                    )
                )
