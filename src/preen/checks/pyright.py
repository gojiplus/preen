        ...
        try:
            ...
            return [
                Issue(
                    ...
                    explanation=(
                        "Pyright returned invalid JSON, possibly due to a "
                        "configuration error"
                    ),
                )
            ]
    ...
    def _get_explanation_for_rule(self, rule: str, severity: str) -> str:
        ...
        rule_explanations = {
            "reportMissingTypeStubs": (
                "Missing type stubs for imported library. Consider installing "
                "a types package or adding type: ignore."
            ),
            "reportUnknownMemberType": (
                "Unknown member type. Add type annotations to improve type safety."
            ),
            "reportUnknownVariableType": (
                "Unknown variable type. Add type annotations for better checking."
            ),
            "reportMissingParameterType": (
                "Missing parameter type annotation. Add type hints for parameters."
            ),
            "reportMissingReturnType": (
                "Missing return type annotation. Add return type hints."
            ),
            "reportUnusedImport": (
                "Unused import detected. Remove unused imports to clean up code."
            ),
            "reportUnusedVariable": (
                "Unused variable detected. Remove it or prefix with underscore."
            ),
            "reportIncompatibleMethodOverride": (
                "Method override is incompatible with the base class. Fix the "
                "signature to match the parent."
            ),
            "reportGeneralTypeIssues": (
                "General type issue detected. Review type annotations and usage."
            ),
        }
        ...
        return (
            f"{base_explanation}. Static type checking helps catch bugs early "
            ...
            case 2 | _ if result.stderr and not result.stdout:
                ...
                issues.append(
                    Issue(
                        ...
                        impact=Impact.IMPORTANT,
                        explanation=(
                            "pyright could not complete type checking, "
                            "possibly due to configuration issues"
                        ),
                    )
                )
            case _:
